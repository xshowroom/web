        ...
        function (
            ...
            var init = function () {
                Order.findOne({
                    ...
                    res.data.order_status = parseInt(res.data.order_status, 10);
                    ...
                    for (var id in res.data.productions) {
                        ...
                        for (var i = 0, len = info.detail.length; i < len; i++) {
                            ...
                            quantity += parseInt(info.detail[i].buy_num, 10);
                        }
            ...
            $element.on('change', '#invoice-file', function (e) {
                ...
                if (files[0].type !== 'application/pdf') {
                    $modal({
                        title: $filter('translate')('modal__title__ERROR'),
                        content: $filter('translate')('modal__msg__error__PDF_TYPE_ERROR'),
                        show: true
                    });
                    $scope.$apply();
                    self.val('');
                ...
                if (files[0].size / 1024 / 1024 > 2) {
                    $modal({
                        title: $filter('translate')('modal__title__ERROR'),
                        content: $filter('translate')('modal__msg__error__PDF_SIZE_ERROR'),
                        show: true
                    });
                    $scope.$apply();
                    self.val('');
                ...
                uiUploader.startUpload({
                    ...
                    onCompleted: function (file, response) {
                        if (!timeout) {
                            $modal({
                                title: $filter('translate')('modal__title__ERROR'),
                                content: $filter('translate')('modal__msg__error__PDF_TIMEOUT'),
                                show: true
                            });
                            $scope.$apply();
                            self.val('');
                        ...
                        if (response.status !== 0) {
                            $modal({
                                title: $filter('translate')('modal__title__ERROR'),
                                content: $filter('translate')('modal__msg__error__PDF_UPLOAD_ERROR'),
                                show: true
                            });
                            $scope.$apply();
                            self.val('');
            ...
            $scope.updateInvoice = function () {
                if (!$scope.order.invoice_url) {
                    $modal({
                        title: $filter('translate')('modal__title__ERROR'),
                        content: $filter('translate')('modal__msg__error__INVOICE_ERROR'),
                        show: true
                    });
                    return;
                }
                Order.updateInvoice({
                    ...
                    $modal({
                        title: $filter('translate')('modal__title__SUCCESS'),
                        content: $filter('translate')('modal__success__INVOICE_UPLOADED'),
                        show: true
                    });
                });
            };
            ...
            $scope.updateShipNo = function () {
                if (!$scope.order.shipNo) {
                    $modal({
                        title: $filter('translate')('modal__title__ERROR'),
                        content: $filter('translate')('modal__msg__error__SHIP_INFO'),
                        show: true
                    });
                    return;
                }
            ...
            $scope.updateShipAmountComments = function () {
                if (!$scope.order.shipNo && !/^\d+$/.test($scope.order.shipAmount)) {
                    $modal({
                        title: $filter('translate')('modal__title__ERROR'),
                        content: $filter('translate')('modal__msg__error__SHIP_FEE'),
                        show: true
                    });
                    return;
                }
